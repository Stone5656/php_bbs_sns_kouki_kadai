...
<body>
  ...
  <div style="margin-bottom: 1em;">
    絞り込み<br>
    <form method="GET">
      名前: <input type="text" name="name" value="<?= htmlspecialchars($_GET['name'] ?? '')?>">
      生まれ年: <input type="text" name="year_from" value="<?= htmlspecialchars($_GET['year_from'] ?? '')?>">年
      ~
      <input type="text" name="year_until" value="<?= htmlspecialchars($_GET['year_until'] ?? '')?>">年
