...
$select_sth->execute([
  ':followee_user_id' => $followee_user['id'], // フォローされている側(フォロー対象)
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') { // フォームでPOSTした場合は実際のフォロー解除処理を行う
  ...
  $insert_result = $insert_sth->execute([
    ':followee_user_id' => $followee_user['id'], // フォロー解除される側(フォロー解除対象)
    ':follower_user_id' => $_SESSION['login_user_id'], // フォロー解除する側はログインしている会員
  ]);
}
?>
...
<div>
  <?= htmlspecialchars($followee_user['name']) ?> さんをフォロー解除しました。<br>
...
<?php else: ?>
<div>
  <?= htmlspecialchars($followee_user['name']) ?> さんをフォロー解除しますか?
  <form method="POST">
