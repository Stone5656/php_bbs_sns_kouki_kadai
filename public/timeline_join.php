...
// JOINでrelationとbbsをuser.idで繋いで、follower_user_idを基に探すようにした
$sql = 'SELECT bbs_entries.*, users.name AS user_name, users.icon_filename AS icon'
  . ' FROM bbs_entries INNER JOIN users ON bbs_entries.user_id = users.id'
  . ' INNER JOIN user_relationships ON users.id = user_relationships.followee_user_id'
  . ' WHERE user_relationships.follower_user_id = :follower_user_id'
  . ' OR users.id = :follower_user_id'
  ...
  <dl style="margin-bottom: 1em; padding-bottom: 1em; border-bottom: 1px solid #ccc;">
    ...
    <dd>
      <a href="/profile.php?user_id=<?= $entry['user_id'] ?>">
        <?php if(!empty($entry['icon'])): // アイコン画像がある場合は表示 ?>
